...
namespace Template.Domain.Common
{
    public abstract class EnumerationBase : IComparable
    {
        ...
        public static T FromName<T>(string name) where T : EnumerationBase
            => Parse<T, string>(name, "name", item => item.Name == name);

        public static string NameFromValue<T>(int value) where T : EnumerationBase
            => FromValue<T>(value).Name;

        public static bool HasValue<T>(int value) where T : EnumerationBase
        {
            try
            {
                FromValue<T>(value);
        ...
        private static T Parse<T, TValue>(TValue value, string description, Func<T, bool> predicate) where T : EnumerationBase
        {
        ...
        public int CompareTo(object? other) => this.Value.CompareTo(((EnumerationBase)other!).Value);
    }
}
